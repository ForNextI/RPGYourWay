export const START_FAQ = [
  ...
  ['How do I add my own character?', 'Add a PDF, JSON, XML, TXT, or Markdown record by browsing for the file, or paste the character information. Custom-character import uses RPG Your Way’s AI services and requires an account with purchased usage.'],
  ...
  ['Does character import cost anything?', 'Custom-character import uses AI and draws from your purchased usage. Ready-to-Play characters do not require AI import, so you can use them to explore campaign setup and the Play page without importing a custom character.'],
  ...
  ['When do I need an account and purchased usage?', 'You can explore ordinary campaign setup with Ready-to-Play characters and inspect the Play page without an account. Begin Your Adventure, custom-character AI import, Detailed Help, and other AI-powered work require an account with purchased usage.'],
] as const
...
A visitor may work through ordinary campaign setup with the Ready-to-Play character library and continue to the Play page without creating an account. Beginning actual AI gameplay requires an account with purchased usage.
...
Custom records may be PDF, JSON, XML, TXT, Markdown, or pasted text. Each file may be up to 8 MB. Custom-character import uses RPG Your Way's AI services and draws from the player's purchased usage. Ready-to-Play characters do not require AI import and may be used to explore the campaign-creation flow without a custom-character charge.
...
Character clarification has two tiers. Required questions are blockers: RPG Your Way cannot use the character correctly until they are answered. Recommended questions are not blockers, but resolving them during setup may prevent later guesses or interruptions. Clarification is part of custom-character AI processing and uses the same purchased usage balance.
...
Detailed Start Page Help uses RPG Your Way's AI services and the player's purchased usage. It is limited to 25 questions per onboarding session and 25 questions per hour from the same connection. Do not discuss unrelated topics. If the answer is not in this knowledge, say so briefly instead of guessing.`

import { claimMultiplayerCharacter, requireMultiplayerUser } from '@/lib/multiplayer/server'
...
export async function POST(request: Request, context: { params: Promise<{ inviteCode: string }> }) {
  try {
    ...
    const session = await claimMultiplayerCharacter(user.id, inviteCode, characterId)

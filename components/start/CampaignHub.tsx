...
export function CampaignHub() {
  ...
  return (
    <details className="start-existing-details start-campaign-hub">
      <summary>Your campaigns, multiplayer controls &amp; imports</summary>
      <div className="start-existing-body">
        <p className="start-existing-lede">Open and manage your current campaigns here. Multiplayer membership and group decisions live with each campaign. Older WardensPC and RPG Your Way exports can be imported here too.</p>
          ...
          <div className="play-entry-grid">
            <section className="play-entry-card" aria-labelledby="saved-adventures-heading">
              <div className="play-entry-card-title">
                ...
                <div>
                  ...
                  <h2 id="saved-adventures-heading">Current campaigns</h2>
                </div>
              </div>
              ...
              {adventures.length ? (
                <div className="play-entry-adventures">
                  {adventures.map((adventure) => {
                    ...
                    return (
                      <article key={adventure.adventure_id} className="play-entry-adventure campaign-hub-card">
                        <button type="button" className="play-entry-adventure-open" disabled={busy} onClick={() => openAdventure(adventure.adventure_id)}>
                          <span>
                            <strong>{adventure.adventure_name}</strong>
                            <small>{adventure.party_names.length ? adventure.party_names.join(', ') : 'Saved party'} · {adventure.storage_source === 'cloud' ? administrationLabel(adventure) : 'This browser · moves to cloud when opened'} · Updated {new Date(adventure.updated_at).toLocaleDateString()}</small>
                          </span>
                          <span className="play-entry-continue"><Play aria-hidden="true" /> Continue</span>
                        </button>

                        {adventure.storage_source === 'cloud' ? (
                          <details className="campaign-control-details" onToggle={(event) => { if (event.currentTarget.open) void ensureGovernance(adventure.adventure_id) }}>
                            <summary><span><UsersRound aria-hidden="true" />Campaign controls</span><span className="accordion-plus" aria-hidden="true" /></summary>
                            <div className="campaign-control-body">
                              {controlsLoading && !view ? <p className="campaign-control-status"><LoaderCircle className="play-entry-spin" aria-hidden="true" />Loading campaign controls…</p> : null}
                              {view ? (
                                <>
                                  ...
                                  <details className="campaign-danger-details">
                                    <summary>{view.campaign_mode === 'multiplayer' ? 'Leave or delete this campaign…' : 'Delete this campaign…'}</summary>
            ...
            <section className="play-entry-card play-entry-import" aria-labelledby="import-adventure-heading">
              <div className="play-entry-card-title">
                ...
                <div><p className="account-state-label">For older users</p><h2 id="import-adventure-heading">Import an existing adventure</h2></div>
              </div>

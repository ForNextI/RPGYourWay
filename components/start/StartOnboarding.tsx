...
const CHARACTER_TOPICS = ['Backstory influence', 'Player-character romance', 'Secret privacy'] as const
  ...
  return (
    <div className="start-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose() }}>
      <section className={`start-modal${wide ? ' start-modal--wide' : ''}`} role="dialog" aria-modal="true" aria-label={title}>
        <div className="start-modal-heading"><h2>{title}</h2><button type="button" className="start-modal-x" onClick={onClose} aria-label="Close"><X aria-hidden="true" /></button></div>
  ...
  async function continueToPlay() {
    ...
    try {
      ...
      const state: SavedAdventureState = {
        ...
        adventure_id: adventureId, adventure_name: campaignName.trim(), game_master_name: gmName.trim().slice(0, 80),
              ...
              {party.map((member) => (
                <article className="start-party-card" key={member.id}>
                  ...
                  <div className="start-party-card-copy">
                    ...
                    {member.imported ? <small className={`start-status start-status--${member.status}`}>{member.status === 'file-added' ? 'File added' : member.status === 'importing' ? 'Importing…' : member.status === 'needs-required' ? 'Required answers needed' : member.status === 'needs-recommended' ? 'Recommended questions' : member.status === 'error' ? 'Import needs attention' : 'Ready'}</small> : <small className="start-status start-status--ready">Ready</small>}
                    {member.error ? <small className="start-card-error">{member.error}</small> : null}
                  </div>
          ...
          {activeStep === 3 && partyReady ? <section className="start-step start-settings-step" aria-labelledby="questions-heading">
            ...
            <div className={questionMode === 'answer' ? 'start-settings-bezel' : 'start-settings-body'}>
              {questionMode === 'pending' ? <div className="start-question-choice" id="questions-heading"><button type="button" className="start-primary-control start-big-control" onClick={() => { setQuestionMode('answer'); setQuestionIndex(0) }}>Customize</button><button type="button" className="start-info-control start-big-control" onClick={() => setQuestionMode('skip')}>Use default settings</button></div> : questionMode === 'skip' ? <div className="start-complete-plaque"><Sparkles aria-hidden="true" /><div><strong>Using the default gameplay settings.</strong><span>You can still change campaign guidance later.</span></div><button type="button" onClick={() => setQuestionMode('pending')}>Change</button></div> : questionMode === 'complete' ? <div className="start-complete-plaque"><Sparkles aria-hidden="true" /><div><strong>Your campaign guidance is set.</strong><span>Six short questions answered.</span></div><button type="button" onClick={() => { setQuestionMode('answer'); setQuestionIndex(0) }}>Review</button></div> : <div className="start-question-panel">
                ...
                {questionIndex === 0 ? <><h2>How often should your Game Master introduce new events, complications, and opportunities?</h2><RatingControl value={initiative} onChange={setInitiative} low="Mostly follow my lead" high="Keep things coming" /></> : null}
...
function RatingControl({ value, onChange, low, high, label }: { value: number; onChange: (value: number) => void; low: string; high: string; label?: string }) {
  return <div className="start-rating-control">{label ? <strong>{label}</strong> : null}<div className="start-rating-scale">{Array.from({ length: 10 }, (_, index) => index + 1).map((number) => <button type="button" key={number} className={value === number ? 'is-selected' : ''} aria-pressed={value === number} onClick={() => onChange(number)}>{number}</button>)}</div><div className="start-rating-labels"><span>{low}</span><span>{high}</span></div></div>
}
